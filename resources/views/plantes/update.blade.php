<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="edit-plante" tabindex="-1" aria-labelledby="newPlantModalTitle" aria-modal="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered relative w-auto pointer-events-none">
      <form id="u_plante" method="POST" class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
  ...
  <script>
    function editPlante(plante) {
      console.log(plante); 
      $.each( plante, function( key, value ) {
        console.log("#u_%s : %s", key, value); 
        $("#u_"+key).val(value).change();
      });
      $('#edit-plante').modal('show');
    }
